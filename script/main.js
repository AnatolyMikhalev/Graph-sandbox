...
function postLoadPage() {
    ...
    let res = "";
    // Generate list of JavaScript files and cache the result
    //const jsScripts1 = Array.from(document.querySelectorAll("script[src]"))

    // const jsScripts = Array.from(document.querySelectorAll("script[src]"))
    //     .filter(script => script.src.endsWith(".js") && script.src.toLowerCase().indexOf("script/plugins".toLowerCase()) > -1)
    //     .map(script => script.src);

    // const jsScripts = Array.from(document.querySelectorAll("script[src]"))
    //     .filter(script => script.src.endsWith(".js") && script.src.includes("plugins/"))
    //     .map(script => script.src);

    // res = JSON.stringify(jsScripts);

    // // localStorage.setItem(tempFilename, res);

    // console.log(res);

    // var scriptList = JSON.parse(res);
